...
func StrictAuth(j *JWT, logger *log.Logger) app.HandlerFunc {
...
func NoStrictAuth(j *JWT, logger *log.Logger) app.HandlerFunc {
	return func(c context.Context, ctx *app.RequestContext) {
		tokenString := ctx.Request.Header.Get("Authorization")
		if tokenString == "" {
			tokenString = string(ctx.Cookie("accessToken"))
		}
		if tokenString == "" {
...
func recoveryLoggerFunc(ctx *app.RequestContext, logger *log.Logger) {
